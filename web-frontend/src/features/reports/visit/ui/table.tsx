...
import { useVisitReportTable } from '../model/use-table';
const useDummyExportVisitReport = () => {
...
export const VisitReportTable = () => {
    ...
    const handleTableHook = (periodValue: string, dateString?: string) => {
        ...
        const { data, isLoading, error } = useVisitReportTable({
            period: periodToUse,
    ...
    const handleExportHook = () => {
        const { downloadReport, downloadExcelReport, isLoading, error } = useDummyExportVisitReport();
        ...
        const mapPeriodToExportParam = (period: string) => {
            ...
            return (periodMapping[period as keyof typeof periodMapping] ?? 'day') as 'day' | 'week' | 'month' | 'year';
        };
