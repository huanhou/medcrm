...
export function useBroadcastEvent<M>(broadcastEvent: BroadcastEvents<M>, listener: (event: M) => void) {
    useEffect(() => {
        return broadcastEvent.listen(listener);
    }, [broadcastEvent, listener]);
}